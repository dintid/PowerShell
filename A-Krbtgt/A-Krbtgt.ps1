...
Release Notes 
    v1.8 (2025-08-25 MODNI):
    1. Fixed `$MyInvocation.MyCommand.Path` null error:
        - Replaced with safer logic that avoids crash in ISE/unsaved runs
        - Script no longer requires Set-Location to its own folder
   2. Centralized logging:
        - All logs now written to C:\ITM8\A-Krbtgt\
        - Automatic creation of log folder if missing
        - Timestamped log filenames for each run
...
# Set working directory to the script folder (robust across ISE/paste/unsaved runs)
$here = $PSScriptRoot
if (-not $here) {
...
if ($here) {
    ...
    Write-Host "Working directory unchanged (no script path available in this run context)."
}
